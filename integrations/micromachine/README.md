...
| `patches/0068-all-terran-harass-capability-evidence.patch` | Admits `harass_with_units` as a genuine `SquadOrderTypes::Harass` operation, preserves Harass order/radius and lifecycle behavior, includes harassment in production activity, and adds operation-scoped squad-order plus 15-family attempted/executed/effect/blocker evidence to telemetry and the existing Battlefield Commander card. The patch is a required schema-68 build input bound into build identity and source attestation. |
...
through `0068`, runs the runtime CTest contracts, then copies the blackboard
header and generates the embedded
identity header before compilation. The
schema-68 report includes pinned MicroMachine and `s2client-api` commits, every patch
