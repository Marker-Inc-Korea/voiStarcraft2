...
| Gate | Result |
| --- | --- |
| Current Python suite | `2155 passed, 6363 subtests passed` in the local `dev + llm` environment |
...
| Current web operation UX | `196 passed, 296 subtests passed`, including non-blocking SSE publication, replay rollover recovery, atomic operation/overview snapshot acceptance, exact update/operation/generation execution identity, non-authoritative detached registry preservation, concurrent source high-water rejection, retention-plus-one milestone deduplication, truthful `order_issued` versus `action_issued`, cancellation cleanup waiting, stable 24-card reconciliation, lane-move focus, and accessibility motion fallbacks |
